...
/**
 * julia - verify if the Julia set works for the given complex number
 * @max: number of remaining iterations
 * @z0: a complex number to verify the julia set
 * @c: a random point in the space
 *
 * Return: 1 if worked, 0 if not
 */
int julia(unsigned int max, cplx z0, cplx c)
{
	if (max > 0)
		return julia(max - 1, cpladd(cplmul(z0, z0), c), c);
	else if (cplmod(z0) > 2)
		return 1;
	else
		return 0;
...
/**
 * main - makes a .pgm file with the result of Julia set
 *
 * Return: zero
 */
int main()
{
	...
	scanf("%lf", &inf_imag);
		...
		printf("\nPut an arbitrary complex number:\nReal part: ");
		scanf("%lf", &re);
		...
		for (i = 0; i < m; i++)
		{
			for (j = 0; j < n; j++)
			{
				...
				matrix[i][j] = julia(iter, C, cpl);
			}
		}
		fp = fopen("julia.pgm", "w+");
		fputs("P2 \n", fp);
		fprintf(fp, "%d %d \n", n, m);
		fputs("1 \n", fp);
		for(i = 0; i < n; i++)
